...
// The tab is a real path — /ventures, not /#ventures — so each section is its
// own URL: shareable, bookmarkable, and something a crawler can index. Only
// these names exist; [section]/page.tsx generates one route per entry and 404s
// anything else, so nothing here has to guard against junk.

// Read from the path rather than the hash. Leading and trailing slashes are
// stripped, so both /work and /work/ resolve.
...
export default function Portfolio({ section = 'about' }: { section?: string }) {
  ...
  // Scroll restoration has to be taken over for a reload to land sensibly.
  // It defaults to 'auto', so the browser put back the offset from before the
  // reload — and since the tab used to reset to About, that was the FAQ's
  // scroll position applied to a shorter page, which is what landed you near
  // the bottom of About. 'manual' hands the decision here, and every load
  // starts at the top of its own section.
  //
  // popstate rather than hashchange: the section is a path now, and the nav
  // moves between them with pushState, so back and forward have to be followed
  // by re-reading the path.
  useEffect(() => {
